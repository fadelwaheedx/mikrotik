...
export const MikrotikLogic = {
     ...
     * @param {number} size - File size in GB
     * @param {number} speed - Speed in Mbps
     * @returns {string} Formatted time string
     */
    calculateBandwidth: (size, speed) => {
        if (!size || !speed) return "Please enter valid numbers.";

        const sizeBits = size * 8 * 1024 * 1024 * 1024; // GB to bits
        const speedBits = speed * 1000 * 1000; // Mbps to bits/s
    ...
    calculateSubnet: (ip, cidr) => {
        // Basic IP Calc Logic (Simplified for text output)
        // In a real app, use a library like 'ip-subnet-calculator'
        // Here we assume the input is validated.

        return `# Subnet Analysis for ${ip}/${cidr}
---------------------------------------------
# Note: For precise binary calculation, integration with
# a dedicated IP library is recommended.

Address:   ${ip}
Netmask:   /${cidr}
Range:     (Calculation requires 'ip-subnet-calculator' lib)`;
    },

    /**
     * Generate DNS over HTTPS (DoH) Script
     * @param {string} url
     * @returns {string}
     */
    generateDoh: (url) => {
        const timestamp = new Date().toLocaleString();
        let script = `# Generated by X Rebuild on ${timestamp}\n`;
        script += `# DNS over HTTPS Configuration\n\n`;

        script += `/tool fetch url="https://curl.se/ca/cacert.pem" mode=https dst-path=cacert.pem\n`;
        script += `/certificate import file-name=cacert.pem passphrase=""\n`;
        script += `/ip dns set verify-doh-cert=yes allow-remote-requests=yes servers=1.1.1.1,8.8.8.8\n`;
        script += `/ip dns set use-doh-server="${url}"\n`;
