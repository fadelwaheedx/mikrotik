...
export const MikrotikLogic = {
    ...
    /**
     * Generates Content Routing (Address Lists) Script
     *
     * @param {Array} apps - Array of app IDs (e.g., ['youtube', 'tiktok'])
     * @param {string} listName - Name of the address list (e.g., 'media_streaming')
     * @returns {string} Generated RouterOS script
     */
    generateContentRouting: (apps, listName) => {
        if (!apps || apps.length === 0 || !listName) {
            return "# Error: Select at least one app and provide a List Name.";
        }

        const timestamp = new Date().toLocaleString();
        let script = `# Generated by X Rebuild on ${timestamp}\n`;
        script += `# Content Routing Address Lists for: ${apps.join(', ')}\n\n`;
        ...
        apps.forEach(app => {
            const appDomains = domains[app];
            if (appDomains) {
                script += `# ${app.charAt(0).toUpperCase() + app.slice(1)}\n`;
                appDomains.forEach(domain => {
                    script += `add list=${listName} address=${domain} comment="${app}"\n`;
                });
            }
        });

        script += `\n# Note: To use these lists, create Mangle rules marking connection/routing based on dst-address-list="${listName}"\n`;
